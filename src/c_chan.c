...
static chan_rv chan_buf_recv( struct channel *chan, void *recv_buf,
  ...
  while ( chan->buf.ring_len == 0 && rv == CHAN_OK ) {
    if ( chan->is_closed )
      ...
      rv = chan_wait( chan, &chan->not_empty, &chan->recv_wait_cnt, timeout );
  } // while
...
static chan_rv chan_buf_send( struct channel *chan, void const *send_buf,
  ...
  while ( rv == CHAN_OK ) {
    if ( chan->is_closed )
      ...
      rv = chan_wait( chan, &chan->not_full, &chan->send_wait_cnt, timeout );
  } // while
 ...
 * expired.
 */
static chan_rv chan_wait( struct channel *chan, pthread_cond_t *cond,
                          unsigned *pwait_cnt,
                          struct timespec const *timeout ) {
  assert( chan != NULL );
  assert( cond != NULL );
  assert( pwait_cnt != NULL );

  if ( timeout == NULL ) {
    ++*pwait_cnt;
    PTHREAD_COND_WAIT( cond, &chan->mtx );
    --*pwait_cnt;
    return CHAN_OK;
  }
  ...
  ++*pwait_cnt;
  int const rv = pthread_cond_timedwait( cond, &chan->mtx, &abs_time );
  --*pwait_cnt;
  switch ( rv ) {
    case 0:
      return CHAN_OK;
